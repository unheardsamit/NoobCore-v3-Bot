...
module.exports = {
        ...
        ncStart: async ({ args, message, api, threadModel, userModel, dashBoardModel, globalModel, threadsData, usersData, dashBoardData, globalData, commandName, event, getLang }) => {
                ...
                const loadScripts = require(process.cwd() + "/core/login/loadScripts.js");
                const unloadScripts = require(process.cwd() + "/core/login/unloadScripts.js");

                if (args[0] == "load" && args.length == 2) {
                        if (!args[1])
                                return message.reply(getLang("missingFileName"));
                        await loadScripts(api, threadModel, userModel, dashBoardModel, globalModel, threadsData, usersData, dashBoardData, globalData, (text) => `LOAD ${text}`);
                        message.reply(getLang("loaded", args[1]));
                }
                else if ((args[0] || "").toLowerCase() == "loadall" || (args[0] == "load" && args.length > 2)) {
                        await loadScripts(api, threadModel, userModel, dashBoardModel, globalModel, threadsData, usersData, dashBoardData, globalData, (text) => `LOADALL ${text}`);
                        const count = global.noobCore.eventCommands.size;
                        message.reply(getLang("loadedSuccess", count));
                }
